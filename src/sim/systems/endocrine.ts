...
export function stepEndocrine(s: SimState, dt: number): void {
  ...
  for (const def of HORMONES) {
    ...
    if (!h) {
      ...
      h = { level: def.baseline, secretion: 0, activity: 0 };
      e.hormones[def.id] = h;
    }
...
function driverValue(s: SimState, signal: string): number {
  switch (signal) {
    ...
    case 'serumPotassium':
      return Math.max(0, (s.chem.k - 4.0) / 2.0);
    ...
    case 'hypovolaemia': {
      ...
      const deficit = 1 - s.cardio.bloodVolume / 5000;
